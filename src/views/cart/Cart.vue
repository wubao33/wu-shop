          ...
          <div class="shopCartListCon" v-for="(goods,index) in shopCart" :key="goods.id">
            <div class="left">
              <a href="javascript:;"
                 ...
                 @click.stop="singerSelected(goods.id)"></a>
            </div>
            <div class="center">
            ...
            <div class="right">
              ...
              <div class="bottomContent">
                ...
                <div class="shopDeal">
                  <span @click="deleteShop(goods.id,goods.num)">-</span>
                  <input disabled type="number" v-model="goods.num">
                  <span @click="addShop(goods.id,goods.name,goods.num,goods.small_image,goods.price)">+</span>
                </div>
              </div>
            </div>
          </div>
...
export default {
  ...
  methods: {
    //购物车商品数量加减
    ...mapMutations(['DELECT_CART', 'ADD_GOODS', 'SINGER_SELECT', 'ALL_SELECT','CLEAR_CART','INIT_SGOP_CART']),
    deleteShop(goodsId, goodsNum) {
      if (goodsNum > 1) {
        this.DELECT_CART({goodsId});
      } else if (goodsNum === 1) {
        Dialog.confirm({
          title: '温馨提示',
          message: '确定删除该商品吗？',
    ...
    addShop(goodsId, goodsName, smallImage, goodsPrice) {
      this.ADD_GOODS({
        goodsId,
        goodsName,
        smallImage,
      ...
      // this.INIT_SGOP_CART();
      // console.log(this.shopCart[goodsId]);
      console.log(Object.values(this.shopCart[goodsId]))
    },
    //选中
    singerSelected(goodsId) {

      this.SINGER_SELECT({goodsId});
    },
    selectedAll(isSelected) {

      this.ALL_SELECT({isSelected});
    },
    clearCart(){
      Dialog.confirm({
        title: '温馨提示',
